...
export default class Cash extends React.Component<props, state> {
  ...
  render() {
    ...
    const bankNames = (paymentData && paymentData.activeMapping.length) ? getPaymentModeByName("cash", paymentData.activeMapping) : [];
    const { amountBean, activeMapping, alertMessage, amountType, channelId, clientCode, clientId, clientLogo, clientName, clientTxnid, donationFlag, donationList, giftCardFlag, isClientLogoFlag, isCommercialFlag, isPartnerBankLogo, isVpaVerify, message, partnerBankLogo, payerEmail, payerMobNumber, payerName, requestAmount, spTxnId } = this.props.paymentData;
        ...
        <div className="activity methods">
          <div className="form form-collapsible">
            <div className="form-header">
              <h3 className="form-heading text-dark mx-2">Cash Payment</h3>
            </div>
            <div className="flex payMethods">
              <a onClick={() => this.props.updateState(54)} className="paymentMethod">
                <img src="https://s3.amazonaws.com/sabpaisa/cash-active.svg" alt="" />
                <span>Cash</span>
              </a>
            ...
            <Form>
              <Select
                ...
                defaultValue={{ label: 'Select Your Bank...', value: {}, key: 2 }}
                ...
                options={bankNames.map((value: any, id: any) => {
                  return {
              ...
              {
                !processing ?
                  <Button disabled = {!isShowButton} onClick={this.makeApiReqCashPayment} className="primaryButton">Proceed to Pay</Button> :
                  <Button className="primaryButton">Processing . . .</Button>
              }
